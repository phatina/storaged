...
/**
 * SECTION:storagedobjectinfo
 * @title: StoragedObjectInfo
 ...
 * storaged_client_get_object_info() to get #StoragedObjectInfo objects.
 * Note that #StoragedObjectInfo is an immutable object; once it has
 * been created it cannot be modified further.
 *
 * The <link
 * linkend="gdbus-property-org-storaged-Storaged-Block.HintName">HintName</link>
 * and/or <link
 * linkend="gdbus-property-org-storaged-Storaged-Block.HintName">HintIconName</link>
 * propreties on associated #StoragedBlock interfaces (if any) may
 * influence what storaged_object_info_get_icon() and
 * storaged_object_info_get_media_icon() returns.
 *
 * The value return by storaged_object_info_get_one_liner() is designed
 ...
 * StoragedObjectInfo:
 *
 * The #StoragedObjectInfo structure contains only private data and
 * should only be accessed using the provided API.
 *
 * Since: 2.1
 */
struct _StoragedObjectInfo
{
...
struct _StoragedObjectInfoClass
{
  GObjectClass parent_class;
};

G_DEFINE_TYPE (StoragedObjectInfo, storaged_object_info, G_TYPE_OBJECT);

/* ---------------------------------------------------------------------------------------------------- */

static void
storaged_object_info_finalize (GObject *object)
{
  StoragedObjectInfo *info = STORAGED_OBJECT_INFO (object);
  ...
  G_OBJECT_CLASS (storaged_object_info_parent_class)->finalize (object);
}

static void
storaged_object_info_init (StoragedObjectInfo *info)
{
}

static void
storaged_object_info_class_init (StoragedObjectInfoClass *klass)
{
  GObjectClass *gobject_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize     = storaged_object_info_finalize;
}


static StoragedObjectInfo *
storaged_object_info_new (StoragedObject *object)
{
  StoragedObjectInfo *ret;

  g_return_val_if_fail (object == NULL || STORAGED_IS_OBJECT (object), NULL);

  ret = g_object_new (STORAGED_TYPE_OBJECT_INFO, NULL);
...
static void
storaged_client_get_object_info_for_block (StoragedClient     *client,
                                           StoragedBlock      *block,
                                           ...
                                           StoragedObjectInfo *info)
{
  ...
  size = storaged_block_get_size (block);
  if (size > 0)
    size_str = storaged_client_get_size_for_display (client, size, FALSE, FALSE);
  ...
  info->name = storaged_block_dup_preferred_device (block);
  if (size_str != NULL)
    {
...
static void
storaged_client_get_object_info_for_loop (StoragedClient     *client,
                                          StoragedLoop       *loop,
                                          StoragedBlock      *block,
                                          StoragedPartition  *partition,
                                          StoragedObjectInfo *info)
{
  ...
  info->icon = g_themed_icon_new_with_default_fallbacks ("drive-removable-media");
  info->icon_symbolic = g_themed_icon_new_with_default_fallbacks ("drive-removable-media-symbolic");
  info->name = storaged_loop_dup_backing_file (loop);
  if (size_str != NULL)
    {
...
static void
storaged_client_get_object_info_for_mdraid (StoragedClient     *client,
                                            ...
                                            StoragedObjectInfo *info)
{
  ...
  block = storaged_client_get_block_for_mdraid (client, mdraid);

  size = storaged_mdraid_get_size (mdraid);
  if (size > 0)
    size_str = storaged_client_get_size_for_display (client, size, FALSE, FALSE);

  name = storaged_mdraid_get_name (mdraid);
  s = strstr (name, ":");
  ...
  level = storaged_mdraid_get_level (mdraid);
  if (size_str != NULL)
    {
...
static void
storaged_client_get_object_info_for_drive (StoragedClient     *client,
                                           StoragedDrive      *drive,
                                           StoragedPartition  *partition,
                                           StoragedObjectInfo *info)
{
  ...
  gchar *s;
  const gchar *cs;
  StoragedBlock *block_for_partition = NULL;

  g_return_if_fail (STORAGED_IS_DRIVE (drive));

  size_str = NULL;

  vendor = storaged_drive_get_vendor (drive);
  model = storaged_drive_get_model (drive);
  size = storaged_drive_get_size (drive);
  media_removable = storaged_drive_get_media_removable (drive);
  media_available = storaged_drive_get_media_available (drive);
  rotation_rate = storaged_drive_get_rotation_rate (drive);
  if (size > 0)
    size_str = storaged_client_get_size_for_display (client, size, FALSE, FALSE);
  media = storaged_drive_get_media (drive);
  media_compat = storaged_drive_get_media_compatibility (drive);
  ...
  if (storaged_drive_get_optical_blank (drive))
    {
  ...
  else if (storaged_drive_get_optical_num_audio_tracks (drive) > 0 &&
           storaged_drive_get_optical_num_data_tracks (drive) > 0)
    {
  ...
  /* Apply STORAGED_NAME, STORAGED_ICON_NAME, STORAGED_SYMBOLIC_ICON_NAME hints, if available */
  block = storaged_client_get_block_for_drive (client, drive, TRUE);
  if (block != NULL)
    {
      cs = storaged_block_get_hint_name (block);
      ...
      cs = storaged_block_get_hint_icon_name (block);
      if (cs != NULL && strlen (cs) > 0)
        {
          g_clear_object (&info->icon);
          g_clear_object (&info->media_icon);
      ...
      cs = storaged_block_get_hint_symbolic_icon_name (block);
      if (cs != NULL && strlen (cs) > 0)
        {
          g_clear_object (&info->icon_symbolic);
          g_clear_object (&info->media_icon_symbolic);
  ...
  if (partition != NULL)
    {
      ...
      if (object_for_partition != NULL)
        block_for_partition = storaged_object_peek_block (STORAGED_OBJECT (object_for_partition));
    }
  ...
  if (block != NULL)
    {
      const gchar *drive_revision = storaged_drive_get_revision (drive);
      if (strlen (drive_revision) > 0)
        {
          ...
          info->one_liner = g_strdup_printf (C_("one-liner-drive", "%s — %s [%s] (%s)"),
                                             ...
                                             info->name,
                                             drive_revision,
                                             storaged_block_get_preferred_device (block_for_partition));
        }
      else
        {
  ...
  info->sort_key = g_strdup_printf ("00_drive_%s", storaged_drive_get_sort_key (drive));
 ...
 *
 * Gets information about a #StoragedObject instance that is suitable to
 * present in an user interface. Information is returned in the
 * #StoragedObjectInfo object and is localized.
 *
 * Returns: (transfer full): A #StoragedObjectInfo instance that should be freed with g_object_unref().
 *
 * Since: 2.1
 */
StoragedObjectInfo *
                                 ...
                                 StoragedObject        *object)
{
  ...
  ret = storaged_object_info_new (object);
  drive = storaged_object_get_drive (object);
  block = storaged_object_get_block (object);
  loop = storaged_object_get_loop (object);
  partition = storaged_object_get_partition (object);
  mdraid = storaged_object_get_mdraid (object);
  if (drive != NULL)
    {
      storaged_client_get_object_info_for_drive (client, drive, NULL, ret);
    }
  else if (mdraid != NULL)
    {
      storaged_client_get_object_info_for_mdraid (client, mdraid, NULL, ret);
    }
  else if (block != NULL)
    {
      drive = storaged_client_get_drive_for_block (client, block);
      if (drive != NULL)
        {
          storaged_client_get_object_info_for_drive (client, drive, partition, ret);
          goto out;
        }

      mdraid = storaged_client_get_mdraid_for_block (client, block);
      if (mdraid != NULL)
        {
          storaged_client_get_object_info_for_mdraid (client, mdraid, partition, ret);
          goto out;
        }

      if (loop != NULL)
        storaged_client_get_object_info_for_loop (client, loop, block, partition, ret);
      else
        storaged_client_get_object_info_for_block (client, block, partition, ret);
    }
 out:
  g_clear_object (&loop);
 ...
 *
 * Gets information about a #StoragedDrive object that is suitable to
 * present in an user interface. The returned strings are localized.
 *
 * Deprecated: 2.1: Use storaged_client_get_object_info() instead.
 */
void
                                ...
                                GIcon        **out_media_icon)
{
  ...
  info = storaged_object_info_new (NULL);
  storaged_client_get_object_info_for_drive (client, drive, NULL, info);
...
/**
 * storaged_object_info_get_object:
 * @info: A #StoragedObjectInfo.
 *
 * Gets the #StoragedObject that @info is for
 *
 ...
 */
StoragedObject *
storaged_object_info_get_object (StoragedObjectInfo *info)
{
  g_return_val_if_fail (STORAGED_IS_OBJECT_INFO (info), NULL);
  return info->object;
}

/**
 * storaged_object_info_get_name:
 * @info: A #StoragedObjectInfo.
 *
 * Gets the name.
 *
 ...
 */
const gchar *
storaged_object_info_get_name (StoragedObjectInfo *info)
{
  g_return_val_if_fail (STORAGED_IS_OBJECT_INFO (info), NULL);
  return info->name;
}
...
/**
 * storaged_object_info_get_description:
 * @info: A #StoragedObjectInfo.
 *
 ...
 */
const gchar *
storaged_object_info_get_description (StoragedObjectInfo *info)
{
  g_return_val_if_fail (STORAGED_IS_OBJECT_INFO (info), NULL);
 ...
 */
GIcon *
storaged_object_info_get_icon (StoragedObjectInfo *info)
{
  g_return_val_if_fail (STORAGED_IS_OBJECT_INFO (info), NULL);
  return info->icon;
}
...
/**
 * storaged_object_info_get_icon_symbolic:
 * @info: A #StoragedObjectInfo.
 *
 ...
 */
GIcon *
storaged_object_info_get_icon_symbolic (StoragedObjectInfo *info)
{
  g_return_val_if_fail (STORAGED_IS_OBJECT_INFO (info), NULL);
...
/**
 * storaged_object_info_get_media_description:
 * @info: A #StoragedObjectInfo.
 *
 ...
 */
const gchar *
storaged_object_info_get_media_description (StoragedObjectInfo *info)
{
  g_return_val_if_fail (STORAGED_IS_OBJECT_INFO (info), NULL);
 ...
 */
GIcon *
storaged_object_info_get_media_icon (StoragedObjectInfo *info)
{
  g_return_val_if_fail (STORAGED_IS_OBJECT_INFO (info), NULL);
...
/**
 * storaged_object_info_get_media_icon_symbolic:
 * @info: A #StoragedObjectInfo.
 *
 ...
 */
GIcon *
storaged_object_info_get_media_icon_symbolic (StoragedObjectInfo *info)
{
  g_return_val_if_fail (STORAGED_IS_OBJECT_INFO (info), NULL);
...
/**
 * storaged_object_info_get_one_liner:
 * @info: A #StoragedObjectInfo.
 *
 ...
 */
const gchar *
storaged_object_info_get_one_liner (StoragedObjectInfo *info)
{
  g_return_val_if_fail (STORAGED_IS_OBJECT_INFO (info), NULL);
...
/**
 * storaged_object_info_get_sort_key:
 * @info: A #StoragedObjectInfo.
 *
 ...
 */
const gchar *
storaged_object_info_get_sort_key (StoragedObjectInfo *info)
{
  g_return_val_if_fail (STORAGED_IS_OBJECT_INFO (info), NULL);
